...
c     !!! Do not deliver any results of this code for any projects !!!
      ...
      open (unit = 12, file = 'Demo1D_area.dat')
      open (unit = 13, file = 'Demo1D_evmax.dat')
      open (unit = 14, file = 'Demo1D_coord.dat')
...
c     outlet
      ...
      do i = 1,im-1
       fxinner = fxinner - 0.5*(p(i+1)+p(i))*(aix(i+1)-aix(i))
       fxouter = fxouter + pout*(aix(i+1)-aix(i))
      enddo

      fxinner = 0.1*fxinner
      fxouter = 0.1*fxouter
      fnetto  = fxinner+fxouter

      write(11,*) '   '
      write(11,'(A13,F12.2,A5)') 'Thrust intern ',fxinner,' [kp]'
      write(11,'(A13,F12.2,A5)') 'Thrust extern ',fxouter,' [kp]'
      write(11,'(A13,F12.2,A5)') 'Thrust netto  ',fnetto,' [kp]'
      write(11,*) '   '

      write(11,*) 'Results:  v,p,ma '

      do i = 1,im
        write(11,*) i,vx(i),p(i),vx(i)/sqrt(cpzucv*p(i)/u1(i))
      enddo
