...
c     !!! Do not deliver any results of this code for any projects !!!
      ...
      open (unit = 15, file = 'Demo3D_ptin.dat')
...
c       from isentropic relations
        ...
        pinold(j,k) = p(1,j,k)
        ts1         = tt1*(p(1,j,k)/ptin(j,k))**rgzucp
...
c      write(11,*) ' '
c      write(11,*) 'update vx,vy,vz,p,u1'
c      call write_prime
...
c     inlet total pressure profile
     ...
     &   +((1.0-ptvbl)*pt1)*sin(real(j-1)/real(jm-1)*pi)
      ...
      do  k = 1,kh-1
       do j = 1,jm
        ptin(j,k) = ptvbl*pt1
     &   +(ptin(j,kh)-ptvbl*pt1)*sin(real(k-1)/real(km-1)*pi)
       enddo
      enddo
     ...
     &   +(ptin(j,kh)-ptvbl*pt1)*sin(real(k-1)/real(km-1)*pi)
      ...
      do  k = 1,km
       do j = 1,jm
        write(15,*) y(1,j,k),z(1,j,k),ptin(j,k)
       enddo
      enddo
...
c     =====================
      ...
      dimension isize(3,3),pt(id,jd,kd)
      ...
      do   k = 1,km
       do  j = 1,jm
        do i = 1,im
         ...
         ts        = p(i,j,k)/(u1(i,j,k)*rg)
         aq        = cpzucv*rg*ts
         rmaq      = (vx(i,j,k)**2+vy(i,j,k)**2+vz(i,j,k)**2)/aq
         tt        = ts*(1.0+0.5*(cpzucv-1.0)*rmaq)
         pt(i,j,k) = p(i,j,k)/((ts/tt)**cpzurg)
...
c    write flow solution (user must use SIDS-standard names here)
     ...
     & RealSingle,'VelocityZ',vz,index_field,ier)
      call cg_field_write_f(index_file,index_base,index_zone,index_flow,
     & RealSingle,'PressureStagnation',pt,index_field,ier)
